...
def hoursHours_minutesMinutes_secondsSeconds(
        milliseconds: int) -> int:  # Функция для преобразования времени в миллисекунды
...
class Log(QDialog):  # Создаем класс, который показывает нам ошибки, связанные с выбором расширения файла
    # Это здесь нужно для доступа к переменным, методам
    # и т.д. в файле Func_For_MP3Player.py
    def __init__(self):
        super().__init__()
